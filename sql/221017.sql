...
-- 인라인 뷰 : from 절 뒤에 쓰여지는 서브쿼리 => 가상테이블
-- 특정결과로 제한할때
...
-- 3번 고객이 주문한 도서의 최고금액보다 더 비싼 도서를 구입한 주문의 주문번호와 금액을 보이시오.
-- all 연산자는 모든 값이 true
...
-- 데이터의 저장 공간 생성 => CREATE TABLE 테이블이름 (컬럼이름 타입 [제약조건])
...
desc emp01;
insert into emp01 (empno,ename,sal) values (1000,'AN',2000);

select * from emp01;

-- as : 서브쿼리를 이용해서 기존 테이블 구조로 생성, 데이터 저장, 제약 조건은 복사안됨
create table emp02
as 
select * from emp
;
...
-- 테이블의 모든 행을 삭제 : 물리적 삭제 되돌릴 수 없음
truncate table emp03;

--테이블의 이름 변경 : rename 기존테이블이용 to 새로운이름
...
insert into emp01(empno, ename, job, deptno) values(1000,'AN','DEV',10);    
insert into emp01(empno, ename, job, deptno) values(1001,'JANG','DEVs',20);
insert into emp01(empno, ename, job, deptno) values(null,'AN','MANAGER',20);
select * from emp01;
desc emp01;
...
insert into emp01 values(1111,'SON','MANAGER',1000,'M',20);
...
insert into emp01 (empno, ename, sal, gender, job, deptno) values(1111,'KING',1000,'M','MANAGER',10);
insert into emp01 (empno, ename, sal, gender, job, deptno, hiredate) values(1111,'KING',1000,'M','MANAGER',10,sysdate);
select * from emp01;