...
function TodoModifyComponent(id, setResult, moveToList, moveToBack) {
    ...
    return (
        <>
            ...
            <Box sx={{p:1}}>
                <TextField variant="outlined" fullWidth name={'id'} label={'ID'} value={String(todo.id)}></TextField>
            </Box>
            ...
            <Box sx={{p:1}}>
                <FormControlLabel control={<Checkbox aria-checked={Boolean(todo.complete)} onChange={(e) => changeTodoComplete(e)} />} label={'Complete'}  />
            </Box>

            <Box sx={{p:1}} display={"flex"} justfyContent={"right"}>
