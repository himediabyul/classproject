...
//                        .deptno(30)
    ...
    public void empPagingTest(){
        ...
        log.info("result >>>>> 전체 게시물 갯수 " + result.getTotalElements());
        log.info("result >>>>> 전체 페이지 갯수 " + result.getTotalPages());
        log.info("result >>>>> 현재 요청 페이지 번호 " + result.getNumber());
        log.info("result >>>>> 페이지당 표현할 항목의 갯수 " + result.getSize());
        log.info("result >>>>> 현재 페이지에 출력된 항목의 갯수 " + result.getNumberOfElements());
